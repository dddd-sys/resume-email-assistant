...
如果这次投递想额外交代 AI 强调某段经历，可以把提示词直接写在命令后面：

```bash
./生成邮件 "请强调我在脉冲 Web3 交易所的经历如何符合 JD 里的 Web3 要求"
```

这条提示词会保存到 `inbox/ai_prompt.txt`，后续运行 `./发送邮件` 时会自动沿用，保证发送内容和预览内容一致。如果你重新运行 `./生成邮件` 且不写提示词，会清空上一次提示词。
...
如果刚才生成预览时写了额外 AI 提示词，发送时也会沿用同一条提示词。
...
如果 JD 写了邮件标题或简历命名格式，程序会按 JD 的格式填入你的个人信息。例如：
...
如果 JD 只写了邮件标题格式或只写了简历命名格式，邮件标题和附件名都会沿用这一个格式。配置了百炼 API 时，程序会优先让模型根据 JD 格式生成标题和附件名；如果 API 不可用，则自动使用本地规则兜底。
